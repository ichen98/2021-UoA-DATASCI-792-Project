...
for (k in 2:36) {
  ...
  activeSquad <- matchPos[which(matchPos[, k] != 0), c(1:k)]
  ...
  for (l in playersOnThisDate) {
    player <- which(activeSquad[, 1] == master2018[l, 1])
    master2018[l, 53] <- activeSquad[player, 2]
  }
}
...
for (m in 2:20) {
  if (m %in% 18:20) {
    preferredPos[, m] <- as.factor(preferredPos[, m])
  } else {
    preferredPos[, m] <- as.numeric(preferredPos[, m])
  }
}
...
for (m in replacements) {
  replacementName <- master2018[m, 1]
  master2018[m, 53] <- as.numeric(preferredPos[which(preferredPos[, 1] == replacementName), 18])
}
...
TODO: Remove duplicate rows, figure out what to do with players that didn't play (probably delete them since they won't be associated with a win margin)
...
```{r cleaning 2019 and 2020 data, cache=TRUE}

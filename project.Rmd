...
for (n in 2:20) {
  if (n %in% 18:20) {
    preferredPos[, n] <- as.factor(preferredPos[, n])
  } else {
...
for (p in unique(master2018$Date)) {
  ...
  for (q in matchPlayers[duplicated(matchPlayers)]) {
    ...
    # Currently gets the row number of the duplicate with the highest number of minutes
    highestMinutes <- as.numeric(rownames(dupes[which(dupes$`Duration Total` == max(dupes$`Duration Total`))]))

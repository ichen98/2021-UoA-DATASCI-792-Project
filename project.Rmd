...
Finally, the 2019 and 2020 datasets are combined, since they share the same columns. The 2018 dataset will remain separate for now.

```{r combining the datasets, cache=TRUE}
combinedData <- rbind(master2019, master2020)
```
...
Therefore, the 2018 dataset must be separated by position, and then the positional maximum, minimum and mean can be found. Variable 18, `HIE Rate` is not numeric, so it is left out here. Any performance markers that are shared by the 2019 and 2020 datasets is also left out.
